...
class EngineState:
    ...
    def __init__(
        ...
        self.positions = np.zeros(n_stocks, dtype=np.float64)       # 持仓数量 (N,)
        self.avg_costs = np.zeros(n_stocks, dtype=np.float64)        # 持仓成本价 (N,)
        ...
        # 列定义: [step_idx, stock_idx, side (1=Buy, -1=Sell), amount, price, fee, cash_after]
